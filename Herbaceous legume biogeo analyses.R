...
### Same Question, but using Likelihood Model to incorporate 0-infl data structure
dat.mle<-dat
dat.mle$logfixra<-log(dat$fixra)
...
fit_u_quad <- mle2(ziln_u_quad,
                  start=list(sdlogrv=sstart,p_no_rv=pstart,c=cstart,b=bstart,b2=.01),
                  data=list(rv_dat=rv,ind_dat=ind))
...
fit_p.u_quad <- mle2(ziln_p.u_quad,
                   start=list(sdlogrv=sstart,c=cstart,b=bstart,b2=.01,d=dstart,e=estart),
                   data=list(rv_dat=rv,ind_dat=ind))
print(summary(fit_p.u_quad))

deltaAICs(c(AICc(fit_rv_null,dat.mle),AICc(fit_p_lin,dat.mle),AICc(fit_u_lin,dat.mle),
            AICc(fit_p.u_lin,dat.mle),AICc(fit_u_quad,dat.mle),AICc(fit_p.u_quad,dat.mle)))
...
g<-merge(g,gherb,by="site.plt.yr",all.x=T,all.y=T)


#This creates a dataframe where grazed and ungrazed data are split out by rows to run anovas on grazing treatment
g.herbdf<-data.frame("site.plt.yr"=c(g$site.plt.yr,g$site.plt.yr),
                   "graz.trt"=c(rep("U",776),rep("G",776)),
                   "tot.relcov"=c(g$ungrazed.tot.relcov,g$grazed.tot.relcov),
                   "tot.rich"=c(g$ungrazed.tot.rich,g$grazed.tot.rich),
                   "fix.ra"=c(g$ungrazed.fix.ra,g$grazed.fix.ra),
                   "fix.rr"=c(g$ungrazed.fix.rr,g$grazed.fix.rr))

##########################################################################
# Running MLE models to test for effect of Herbivory on fixer relative abundance assuming 0-inflated lognormal data
...
map('worldHires')
with(dat, points(x=LON, y=LAT, bg='forestgreen', pch=21, cex=1.2))
#dev.off()

#Now with GEx and CoRRE split out
png(filename = "Map of Data Distribution.png", width=8, height=4, units="in", res=300)
map('worldHires')
with(dat[dat$dataset=="GEx",], points(x=LON, y=LAT, bg='blue', pch=21, cex=1.2))
with(dat[dat$dataset=="CoRRE",], points(x=LON, y=LAT, bg='red', pch=21, cex=1.2))
